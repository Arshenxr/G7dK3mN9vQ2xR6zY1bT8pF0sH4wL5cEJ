--========================================================--
-- Westbound.win ScriptModule (Clean + Full Features)
--========================================================--
...
ScriptModule.Init = function(Fluent, SaveManager, InterfaceManager)
    ...
    local function addIfRelevant(obj)
        if (obj:IsA("Decal") or obj:IsA("Texture") or obj:IsA("Sound") or
            obj:IsA("MeshPart") or obj:IsA("UnionOperation") or obj:IsA("Part") or
            obj:IsA("Accessory") or obj:IsA("Mesh") or obj:IsA("SpecialMesh") or
            obj:IsA("ImageLabel") or obj:IsA("ImageButton")) then
    ...
    FullBrightToggle:OnChanged(function(enabled)
        ...
        if enabled then
            OldLighting = {Brightness=Lighting.Brightness, ClockTime=Lighting.ClockTime,
            ...
            lightingChangedConn = Lighting.Changed:Connect(function(prop)
                if FullBrightEnabled and (prop=="Brightness" or prop=="ClockTime" or
                    prop=="FogEnd" or prop=="GlobalShadows" or prop=="Ambient") then
                    applyFullBright()
                end
            end)
        else
    ...
    local customNames = {legendarydirewolf={name="Legendary Direwolf"}, legendarybison={name="Legendary Bison"},
        legendarygrizzlybear={name="Legendary Grizzly Bear"}, legendaryblackbear={name="Legendary Black Bear"},
        legendarywolf={name="Legendary Wolf"}, legendarymoosebull={name="Legendary Moose Bull"},
    ...
    -- Auto Heal
    local AutoHealToggle = ModAssistSectionAssist:AddToggle("AutoHealToggle", {
        Title = "Auto Heal",
        Default = false
    })
    ...
    local function hookInstantContextAction()
        if ICA_Hooked then return end
        ICA_Hooked = true
        for _, v in pairs(getgc(true)) do
            if type(v)=="function" then
                local info = (pcall(function() return debug.getinfo(v) end) and debug.getinfo(v)) or nil
    ...
    ICA_Toggle:OnChanged(function(value)
        ...
        if value then hookInstantContextAction() end
    end)
    ...
    -- Gun Mod
    local GunToggle = ModAssistSectionMods:AddToggle("GunModToggle", {
        Title = "Enable Gun Mods",
        Default = false
    })
    GunToggle:OnChanged(function(value)
        local success, list = pcall(function() return require(ReplicatedStorage.GunScripts.GunStats) end)
        if success and list and value then
            for _, v in pairs(list) do
                pcall(function()
                    v.Spread=0; v.prepTime=0; v.equipTime=0.1; v.MaxShots=math.huge;
                    v.ReloadSpeed=0; v.BulletSpeed=250; v.HipFireAccuracy=0; v.ZoomAccuracy=0;
                    v.camShakeResist=0; v.AutoFire=true
                end)
            end
            print("[Westbound.win] Gun Mods Enabled")
        else
            print("[Westbound.win] Gun Mods Disabled or Failed to Load GunStats")
        end
    end)

    -- Infinite Gallop
    local StaminaToggle = ModAssistSectionMods:AddToggle("InfiniteGallopToggle", {
        Title = "Infinite Gallop Stamina",
        Default = false
    })

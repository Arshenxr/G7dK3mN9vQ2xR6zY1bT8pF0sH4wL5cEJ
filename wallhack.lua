...
if (not getgenv().Ashenxr_westboundwin or not getgenv().Ashenxr_westboundwin.Visuals) then
	return;
end
...
local CrosshairParts, WrappedPlayers = {LeftLine=Drawingnew("Line"),RightLine=Drawingnew("Line"),TopLine=Drawingnew("Line"),BottomLine=Drawingnew("Line"),CenterDot=Drawingnew("Circle")}, {};
function WorldToViewportPoint(...)
	return Camera.WorldToViewportPoint(Camera, ...);
end
local function GetEnv()
	return getgenv().Ashenxr_westboundwin.Visuals;
end
...
local Visuals = {AddESP_Animal=function(Animal)
	local AnimalTable = {Name=Animal.Name,ESP=Drawingnew("Text"),Connections={}};
		...
		if (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and workspace.Animals:FindFirstChild(AnimalTable.Name) and Animal:FindFirstChildOfClass("Humanoid") and Animal:FindFirstChild("HumanoidRootPart") and Animal:FindFirstChild("Head") and GetEnv().Settings.Enabled and GetEnv().Settings.Animals) then
			...
			if OnScreen then
				...
				if AnimalTable.ESP.Visible then
					...
					local Parts, Content = {Health=("(" .. tostring(mathfloor(Animal.Humanoid.Health)) .. ")"),Distance=("[" .. tostring(mathfloor(((Animal.HumanoidRootPart.Position or Vector3zero) - (LocalPlayer.Character.HumanoidRootPart.Position or Vector3zero)).Magnitude)) .. "]"),Name=stringgsub(stringmatch(Animal.Name, "(.+){"), "(%l)(%u)", function(...)
		...
		if (LocalPlayer.Character and Player.Character and Player.Character:FindFirstChildOfClass("Humanoid") and Player.Character:FindFirstChild("HumanoidRootPart") and Player.Character:FindFirstChild("Head") and GetEnv().Settings.Enabled) then
			...
			if (OnScreen and GetEnv().Visuals.ESPSettings.Enabled) then
				...
				if PlayerTable.ESP.Visible then
					...
					local Parts, Content = {Health=("(" .. tostring(mathfloor(Player.Character.Humanoid.Health)) .. ")"),Distance=("[" .. tostring(mathfloor(((Player.Character.HumanoidRootPart.Position or Vector3zero) - (LocalPlayer.Character.HumanoidRootPart.Position or Vector3zero)).Magnitude)) .. "]"),Name=(((Player.DisplayName == Player.Name) and Player.Name) or (Player.DisplayName .. " {" .. Player.Name .. "}"))}, "";
					if GetEnv().Visuals.ESPSettings.DisplayName then
						Content = Parts.Name .. Content;
					end
					if GetEnv().Visuals.ESPSettings.DisplayHealth then
...
end,AddHeadDot=function(Player)
	local PlayerTable = GetPlayerTable(Player);

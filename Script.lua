...
function ScriptModule.Init(Fluent, SaveManager, InterfaceManager, LocalPlayer)
    ...
    local Camera = Workspace:WaitForChild("CurrentCamera", 5) -- รอไม่เกิน 5 วินาที
    if not Camera then
    ...
    createServerButton("Hop Server", function()
        ...
        if bestServer then TeleportService:TeleportToPlaceInstance(PlaceId, bestServer, LocalPlayer) else TeleportService
                :Teleport(PlaceId, LocalPlayer) end
    end)

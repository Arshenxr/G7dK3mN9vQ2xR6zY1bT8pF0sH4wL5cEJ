...
function ScriptModule.Init(Fluent, SaveManager, InterfaceManager, LocalPlayer)
    ...
    local Camera = Workspace:GetService("CurrentCamera")
    ...
    createServerButton("Hop Server", function()
        ...
        if bestServer then TeleportService:TeleportToPlaceInstance(PlaceId, bestServer, LocalPlayer) else TeleportService:Teleport(PlaceId, LocalPlayer) end
    end)
